    ...
    for batch_size in tqdm(feasible_batch_sizes, desc="Optimizing Among Feasible Sizes"):
        ...
        for strategy_idx, (compress_w, compress_c) in enumerate([(False, False), (False, True), (True, True), (True, False)]):
            ...
            if pulp.LpStatus[prob.status] == "Optimal":
                ...
                H = config.input_dim
                S = input_len + gen_len
                latency_per_layer = (min_transfer_time_per_layer - 
                                     total_hidden_state_size * (vars["h_cpu"].varValue * T_cpu_to_gpu + vars["h_disk"].varValue * T_disk_to_gpu))
                ...
                total_latency = (T_compute_gpu_per_layer + latency_per_layer) * num_layers + \
                                (total_hidden_state_size * (vars["h_cpu"].varValue * T_cpu_to_gpu + vars["h_disk"].varValue * T_disk_to_gpu))
...
def estimate_actual_gpu_usage(policy, model_name, input_len, gen_len, compression_factors):
    ...
    # Actual sizes after compression
